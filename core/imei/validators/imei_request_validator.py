...
class ImeiRequestValidator():
    ...
    def validator_parameter_accion_masivo(self, request_accion):
        mensaje_return = ""
        # Se hace una validacion de los parametros lista, operadora y origen
        # para verificar si se esta recibiendo los calores que corresponden
        # segun lo definido en el config.json
        ...
        data = json.load(config)
        # Evaluando Origen
        existe_origen = list(filter(lambda x: x["valor"] == request_accion, data["valor_accion_masivo"]))
        if len(existe_origen) <= 0:
    ...
    def validator_parameter_list_check(self, request_list_check):
        mensaje_return = ""
        # Se hace una validacion de los parametros lista, operadora y origen
        # para verificar si se esta recibiendo los calores que corresponden
        # segun lo definido en el config.json
        ...
        data = json.load(config)
        # Evaluando Origen
        existe_origen = list(filter(lambda x: x["valor"] == request_list_check, data["valores_listcheck_imei"]))
        if len(existe_origen) <= 0:
            mensaje_return = "El parametro list tiene un valor que no es reconocible en la configuracion de los posibles valores que puede recibir. Revisar la documentacion"
