    ...
    def registrarAccionApiUsuarios(self, data_request, direccion_ip):
            try:
                if not user_api_actions.objects.filter(
                        Q(username=data_request["username"]) & Q(target=data_request["target"]) & Q(action=data_request["action"])).exists():
                    ...
                    item_update = user_api_actions.get(username=data_request["user_id"], action=data_request["action"], target=data_request["target"])
